...
| The story + series (`exhibition.js` — told story + side room) | The narrator layer (fetch, cadence, degrade-to-silence) and the series side room (pill, crossing, lane, close) | `EX-STORY*`, `EX-STORY-BEAT`, `EX-SERIES`, INV-46, INV-47, INV-88, INV-89 | `engine/assets/exhibition.js:2378` (`function tellStory`), `:4313` (`function openSide`) |
...
| bake → client | the bake · the door/gallery/zoom/story/quiz/chrome client | `exhibition_data.json` (works, captions, palettes) + `config.json` (flags, feel-knobs, the `experiments` registry — `quiz_chip_copy` when the quiz ships, `arms·flag·metric·salt`, `EX-QUIZ-COPY`/`EX-AB` (the sibling `quiz_arm` split RETIRED 2026-07-28, SPEC.md tombstone) — and the `lang_geo` country→tongues map from `site.json`, `EX-LANG-GEO`) | the bake (`engine/build.py:855`, `:1011`) |
