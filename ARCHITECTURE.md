...
| The chrome + compose (`exhibition.js` — sound, faceSync, pulse) | The ambient player, the one-page-shape face lock shared by every standing face, and the analytics event registry | `EX-CHROME`, `EX-SOUND*`, `EX-PULSE`, `EX-AB`, `EX-MEMORY`, `EX-EDGE-GUARD` (client half), INV-22..INV-25, INV-27, INV-41, INV-43, INV-67, INV-70, INV-90, INV-91, INV-96 | `engine/assets/exhibition.js:2889` (`function faceSync`), `:75` (`function pulse`), `:3647` (`(function sound()`) |
...
| The edge worker (`engine/assets/worker.js`) | Answers `/api/story`, `/api/quiz`, `/api/i18n`; holds the model API key, the baked-in private story fragments and quiz answers, the KV cache, and the three money fences | `EX-STORY-EDGE`, `EX-QUIZ-EDGE`, `EX-I18N`, `EX-EDGE-GUARD`, INV-26, INV-51, INV-59, INV-68 | `engine/assets/worker.js:92` (`export default {`), `:372` (`async function quiz`), `:188` (`async function story`) |
...
| The test suite (`tests/run_all.py` + `tests/test_*.py`) | Runs all 32 named suites and gates green/red; each `test_*.py` traces to the anchors it asserts | (infrastructure — traceability lives per-suite, not summarized here) | `tests/run_all.py:20` (`SUITES = [...]`) |
...
| bake → client | the bake · the door/gallery/zoom/story/quiz/chrome client | `exhibition_data.json` (works, captions, palettes) + `config.json` (flags, feel-knobs, experiments registry) | the bake (`engine/build.py:855`, `:1011`) |
