...
| The story + series (`exhibition.js` — told story + side room) | The narrator layer (fetch, cadence, degrade-to-silence) and the series side room (pill, crossing, lane, close) | `EX-STORY*`, `EX-STORY-BEAT`, `EX-STORY-LEAD`, `EX-SERIES`, INV-46, INV-47, INV-88, INV-89, INV-108 | `engine/assets/exhibition.js:2378` (`function tellStory`), `:4313` (`function openSide`) |
...
| The chrome + compose (`exhibition.js` — sound, faceSync, pulse) | The ambient player, the one-page-shape face lock shared by every standing face, and the analytics event registry | `EX-CHROME`, `EX-SOUND*`, `EX-SOUND-GREET`, `EX-PULSE`, `EX-AB`, `EX-MEMORY`, `EX-EDGE-GUARD` (client half), INV-22..INV-25, INV-27, INV-41, INV-43, INV-67, INV-70, INV-90, INV-91, INV-96, INV-101 | `engine/assets/exhibition.js:2889` (`function faceSync`), `:75` (`function pulse`), `:3866` (`(function sound()`) |
...
| The client assembler (`engine/assemble_client.py` + `engine/client/`) | The served `exhibition.js` is a committed, GENERATED file: 21 ordered raw line-slice fragments in `engine/client/` joined verbatim (empty-string concat) by an explicit manifest into `engine/assets/exhibition.js`. The generated file is never hand-edited — edits land in a fragment. `build.py` is untouched (the `@@NS@@` tokens ride the join). | (infrastructure — preserves the client byte-for-byte, owns no spec anchor) | `engine/assemble_client.py:1`, `engine/client/` |
| The edge worker (`engine/assets/worker.js`) | Answers `/api/story`, `/api/quiz`, `/api/i18n`, `/api/geo`; holds the model API key, the baked-in private story fragments and quiz answers, the KV cache, and the three money fences. `/api/geo` returns the arriving country (`{c}` from `request.cf.country`, `cf-ipcountry` fallback), `no-store`, dispatched before the `/api/i18n` 404 gate — used only to narrow the language corner (`EX-LANG-GEO`), never stored, never on a beat | `EX-STORY-EDGE`, `EX-STORY-FILL`, `EX-QUIZ-EDGE`, `EX-I18N`, `EX-LANG-GEO`, `EX-EDGE-GUARD`, INV-26, INV-51, INV-59, INV-68, INV-105 | `engine/assets/worker.js:111` (`export default {`), `:455` (`async function quiz`), `:221` (`async function story`), `function geo(req)` |
...
| The test suite (`tests/run_all.py` + `tests/test_*.py`) | Runs every named suite and gates green/red; each `test_*.py` traces to the anchors it asserts | (infrastructure — traceability lives per-suite, not summarized here) | `tests/run_all.py:20` (`SUITES = [...]`) |
...
| bake → client | the bake · the door/gallery/zoom/story/quiz/chrome client | `exhibition_data.json` (works, captions, palettes) + `config.json` (flags, feel-knobs, the `experiments` registry — `arms·flag·metric·salt`, `EX-AB` (both the `quiz_arm` and `quiz_chip_copy` splits RETIRED 2026-07-28, SPEC.md tombstones; the quiz chip's words now ship as fixed copy, `EX-QUIZ-COPY`) — and the `lang_geo` country→tongues map from `site.json`, `EX-LANG-GEO`) | the bake (`engine/build.py:855`, `:1011`) |
...
| bake → about pages | the bake · the CDN | one flat page per baked language (`about.html`, `about/<lang>.html`), each read wholly from the instance copy dictionary; the greet block that rides `exhibition_data.json` is TRIMMED of the page copy on the way, carrying only the `about` entry word (INV-104) | the bake (`engine/build.py`, `render_about`) |
...
| client → worker | the door/story/quiz/chrome client · the edge worker | `/api/story`, `/api/quiz`, `/api/i18n` JSON requests and responses | the worker (`engine/assets/worker.js:368` `storyShape`, `:524` `shape`) |
...
| The bake strips comments from the served `exhibition.css` (2026-07-23) and `exhibition.js` (1.14.0); the source keeps every comment and every served line is a source line, in order | resolved (as-shipped) | this doc's Quality budgets section |
