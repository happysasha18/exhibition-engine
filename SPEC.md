...
The transition is a **cubic ease that arrives at rest and leaves at whatever speed it was entered
at**. A glide begun from a resting walk enters at zero and is the calmest classic shape, both ends
...
curve is ENTERED at the speed the old one was running at, so position and speed are both continuous
and the walk never decelerates to a dead stop in the middle of a frame. A re-time asks the new force's
duration for a WHOLE frame and takes the share of it still left to travel, and it may only bring the
landing NEARER, never push it out — a slow drag re-times four or five times inside one step, and a
re-time that restarted a full-length glide over a shrinking gap would leave the walk crawling. An
entry speed is clamped into the band where the curve stays monotonic, and an entry speed pushing
against the travel is dropped to a rest start rather than lurching backward.
...
transition's heading; a held key is one frame per press. **Touch** is owned by the same **JS animator**.
While the walk owns the input every `touchmove` is `preventDefault`ed, so the native scroll never runs
and a momentum swipe can never fly through several works. The animator writes the position only **after
the finger lifts**, so no live native scroll is ever fought and the iOS jerk-fix holds by construction.
A lift whose net vertical travel reaches the swipe floor (24px) steps **exactly one frame** — a finger
travelling up walks forward, a finger travelling down walks back — while a tap or a shorter drag leaves
the walk where it stood. That step is asked for with no velocity reading, so a swipe rides the full
`glide_ms` clock; the one-gesture-one-frame law (`INV-84`) holds on the touch path and the desktop path
alike, and the force-scales-speed half of it runs on the wheel path today. The cold **door ignores a wheel** — the animator owns the walk only, never the door,
...
for, the highest lower row that does fit is declared instead, and one of those two always takes them:
a crossing spends at most one impossible event (charter shelf 6, and the composer holds the slot shut
against a second), its letters never pass two, and the composition gives its colour voice up rather
than overrun the declared row's own accompaniment ceiling — so a composed plan's counts always stand
INSIDE the row it declares. The code holds a third path for a count no row takes, and it is the
...
pair is looked up, and where the ranking offers none the slot stands exactly as it did.

**And the walk holds its one impossible event for the step it names the culmination.** A walk spends
one impossible event, and a route puts several middles before its crest — each of them granted one by
its own role — so the walk's only event was routinely spent before the crest was reached, and the
crest was then refused for want of it. While the route still holds an uncrossed crest, the walk sends
a step that is not the crest a spent-event log saying the event is not its to spend: the step asks
under its own name and its own letters exactly as before, and only that one slot is closed to it, so a
fold it casts plays as an ordinary letter — which is what a fold after the crest already does. The
hold ends with the crest EDGE being crossed, whichever tier that crossing managed, so a crest the
composer's gate refuses frees the steps after it; and a step at or after the crest is never held. It
is the walk's own intent and not a quota: no count is kept, no pair is looked up, and nothing is
promised to the crest but a slot left unspent. The passage carries one field beside the composer's
`requestedRole`/`realisedTier`/`downgradeReason` saying whether the event stood held at that step,
which is the plain reason a middle before the crest carried none. A **cue role** is what one cue does dramatically
...
lands, the file fails, or the press falls back to arming. A picture still arriving where a tone rectangle does not fit
wears a skeleton shimmer over its box until it paints — the cold closer-look slot, whose box is the
arriving crop itself. Reduced motion holds each mark steady. `INV-48`
...
unit — the same mirror margin the touch pinch uses (~0.92× at rest, `INV-82`). **The target is resolved
...
dismiss margin (about 0.92× of the picture's resting size, `INV-82`), so the in-pinch mirrors the
...
(≥44px) and carries an aria-label; and on a coarse pointer every chrome control answers a PRESS with
its own engaged affordance for the press's duration — the accent fill / glow a mouse gets from hover,
which the hover-gating (`hover:hover`) withholds from a finger — added on `pointerdown` and cleared on
lift or the browser's gesture cancel, hover:none-gated so a pointer device keeps its own hover fill.
*Non-goals:* a general collision solver (each overlay places its own
...
host from config, never a literal.

**A taken copy has two sizes, and the won one is larger.** An ordinary grab is drawn down so its LONG
EDGE is at most `GRAB_MAX_PX` (800 `[default]`) on every screen, whatever tier the walk is displaying,
and the mark is drawn at that size. A copy already smaller than the cap is handed over at its own size.
The **quiz prize** keeps its own road: the pre-marked derivative reaches the visitor through a plain
fetch, with no canvas on the way, so a won picture stays the better file on every screen. An instance
whose ladder tops out above the prize's own size would otherwise hand an ordinary grab a larger file
than the prize. `EX-PROTECT-RES` `INV-56`
...
plus `about`, the one word the entry links wear. The bake hand-writes none of it, and it reads every
language alike: how a tongue came to hold its words is the instance's own authoring concern, settled
before the bake starts. An instance may write the heading and the four paragraphs by hand in one
source language and derive the other tongues from that text with its own authoring command, each
derived language recording the source text it was made from so the instance reads which of its
tongues have fallen behind; `about` and `about_back` stay hand-written in every language. **The
fallback tongue decides whether the feature runs at all:** a dictionary whose fallback language carries no
...
own `about_title`, and one that does not is simply absent from the `hreflang` set. `INV-9`
`INV-102`
...
| `INV-63` | The responsive image ladder: the display-cap bake writes clean 640/960/1280 tiers next to each served image (downscale-only, no mark on a served tier); every surface that hangs a work offers them via a per-work `srcset` — the walk, a door window, a series lane picture, a polaroid on the table, the `/w` page — each handing the `sizes` its own box asks for, so a thumb-sized box pulls a thumb-sized file; the base `src` the untouched fallback; joins only when the cap runs, else byte-identical |
...
| `INV-84` | One continuous input gesture — one wheel burst, one touch swipe, one arrow press — advances EXACTLY ONE frame, always; the gesture's velocity sets that single glide's DURATION within a clamped range (calm ~520ms → sharp ~260ms `[default]`), never a second frame — the earlier two-frame flick allowance is retired. A glide REPLACED in flight — re-timed by a rising wheel sample, or chained to the next frame by a second gesture — is entered at the speed the running glide had, so position and speed are both continuous and the walk never stalls mid-frame; a re-time takes the share of the new force's whole-frame duration still left to travel and may only bring the landing nearer, never push it out. The desktop animator coalesces a whole trackpad burst (rising tail and all) to one step and reads velocity for the speed; the touch path blocks the native scroll and takes its one step when the finger lifts past a 24px swipe floor, asking for that step with no velocity reading. The one-frame-per-gesture half is uniform on the desktop wheel path and the touch swipe path; the velocity half runs on the wheel path today |
| `INV-85` | On a non-touch (desktop) device a trackpad pinch opens and drives the SAME inspect/zoom layer a two-finger touch pinch opens (`INV-75`), on the picture under the POINTER over the same picture selectors (the ZOOM picture-selector set) — and where the pointer is over no picture, the single work then in the viewport (one work per viewport, EX-HANG), else nothing opens — scaling continuously under the same 1×–4× clamp; a `ctrl`+wheel `deltaY` (Blink) and a `gesturechange` scale delta (Safari) accumulate into that clamp, and a pinch-IN past the dismiss threshold (the same mirror margin touch uses, ~0.92× at rest) closes it through the one history step (`INV-83`) and ×/Esc/backdrop/browser-Back all still close; entry and exit mirror by the same FLIP as touch (`INV-82`). The input split is clean: a plain wheel is navigation (EX-GLIDE/`INV-84`), a `ctrl`+wheel / trackpad pinch is zoom (EX-PROTECT hands the gesture over rather than refusing it, latched at the burst's first event); a physical `Ctrl`+mouse-wheel is indistinguishable from a pinch on Blink and drives the zoom discretely; a plain wheel-only mouse never opens the zoom |
...
| `INV-93` | One dismiss margin (~0.92× of the resting size, `INV-82`) serves both the zoom-out and the dismiss across the touch pinch, the trackpad pinch, and the physical modifier wheel, so the in-pinch mirrors the out-pinch under a single value; a release below resting closes, at or above resting holds open |
