...
| `EX-GLIDE` | One input gesture → EXACTLY one centered frame — a cubic ease that arrives at rest and is entered at the running speed, no drift, no mid-frame stall; force scales the single glide's SPEED, never the count (`INV-84`); desktop animator, touch snap; survives a device rotation (`INV-86`) |
