...
overlay, the door covers its own frames); the one non-goal that changed — a wordless bar past the long
wait — is amended by name, numbers still forbidden. The DOOR and the crossing keep their own image
handling (the crossing warms the picked work, so the room opens without a plate; only a cache-evicted or
slow first image falls through here); the side room's lane/polaroid members keep their own lazy render;
the ladder governs the **walk's in-view frame only**, and continues behind any standing face. The knobs
ride `config.exhibition` (`INV-28`), each a beat ×tempo (`INV-33`); the order
...
| `EX-LOAD-2` | The in-flight ladder: black → the work's own tone-plate (past the grace) → plate+bar (past the long wait) → the photo fades in over it, crisp when it beat the plate, graceful when it stood; the arm reads the settled state and arms once; supersedes the lone breath hairline |
