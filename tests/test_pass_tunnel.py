...
check("PASS-TUNNEL the mip chain could not cross, and the measurement that fed it did",
      ...
      and "makeTex" in LAYER and "generateMipmap" not in LAYER,
      "the module uploads its own texture and builds a mip chain on it, then picks the level by "
      "hand because `fract()` breaks the automatic derivatives. The host uploads both works with "
      "LINEAR filtering and no mip chain (pass-layer.js's own `makeTex`), and §1.2's fence leaves "
      "every texture to the host, so an instrument cannot build one. The three footprint "
      "expressions crossed unchanged and are spent on five taps over that footprint instead of on a "
      "level")
