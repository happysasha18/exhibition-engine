...
check("PASS-KAL the mip chain the host does not build is said to be missing, not quietly dropped",
      "THE MIP CHAIN" in SOURCE_TEXT
      and "textureGrad" in LABTXT and "textureGrad" not in REGION
      and "generateMipmap" in LABTXT and "generateMipmap" not in LAYER,
      "the module builds a mip chain and asks for anisotropy, and its whole gradient estimate exists "
      "to pick a level that does not jump at a fold line. This host binds LINEAR with no chain at "
      "all, so the estimate would be inert arithmetic. It is not carried, and what the missing chain "
      "costs — aliasing in the outer rings at a deep sample width — is written down as a request to "
      "the host rather than lost")
...
ASKS_BLOCK = (re.search(r"asks:.*?levels:", REGION, re.S) or [""])[0]
check("PASS-KAL the instrument declares what a pair must read for it to be worth casting",
      'measure: "radial"' in REGION
      and 'floor: "radial"' in REGION and 'floor: "radial_tight"' in REGION
      and 'subType: "ring"' in REGION
      and bool(ASKS_BLOCK) and not re.search(r"\d\.\d", ASKS_BLOCK),
      "both works have to read radial over the collection's own cut-line floor, so the fold opens a "
      "structure the pair carries rather than one imposed on a work that has none; and the ARRIVING "
      "work has to read radial over the tight floor with its own subtype on rings, because rings are "
      "what open into a rosette and spokes turn instead. The floors are NAMED and not copied: the "
      "numbers belong to the collection and no instrument may keep a copy of them")
    ...
    with serve(BENCH) as base:
        with Browser(width=VW, height=VH) as br:
            ...
            if not ready(br):
                ...
                res = m["resources"]
                need = ["id", "api", "arity", "roles", "cuts", "asks", "params", "handles",
