...
if not node_available():
    ...
    if got.get("error"):
        ...
        check(NODE_ROWS[3], not bad and told >= 3,
        ...
        # --- row 4 · the family the composer hands back is the walk's own ------------------------
        m = got["memory"]
        agree = (m["saidForward"] == m["walkForward"] and m["saidBack"] == m["walkBack"]
                 and m["saidAgain"] == m["walkAgain"])
        check(NODE_ROWS[4], agree and bool(m["walkForward"]),
        ...
        # --- row 5 · §4.8's own two questions, asked of the way back ------------------------------
        ...
        # --- row 7 · the open handle ------------------------------------------------------------
        check(NODE_ROWS[7], not sweep["openDriven"],
        ...
        # --- row 8 · the two fences a filled score has to pass -----------------------------------
        check(NODE_ROWS[8], sweep["overByte"] == 0 and sweep["overIntent"] == 0,
        ...
        # --- row 10 · the defaults reproduce the four-value call -----------------------------------
        dd = got["defaults"]
        check(NODE_ROWS[10], dd["spelledSame"] and dd["coreSame"],
        ...
        # --- rows 14-22 · the same repairs, each reverted in a copy ---------------------------------
        ...
        CORNER = 24
        PLANTS = [
            (NODE_ROWS[14], [["if (ROUTE_ROLES.indexOf(role) < 0) {", "if (false) {"]],
             lambda g: g["fences"]["role"]["composed"] is True),
            (NODE_ROWS[15], [["if (odd.length) {", "if (false) {"]],
            ...
            (NODE_ROWS[18], [["if (fits) break;", "break;"]],
