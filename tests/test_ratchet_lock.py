...
SEEDED_STYLE_ERRORS = 26
SEEDED_REDUNDANCY_OPEN = 10
GATED_DOCS = ['SPEC.md']
TIER = 'universal'
...
if __name__ == "__main__":
    # run_all drives every suite as a plain script (SUITES == on-disk test_*.py identity gate in
    # test_site), so this generated pytest file self-runs under pytest when invoked directly.
    # NOTE: a ratchet re-install (adopt/install-ratchet.sh --force) rewrites this file — re-append
    # this block after any re-install.
    import sys
    sys.exit(subprocess.run([sys.executable, "-m", "pytest", "-q", __file__]).returncode)
