...
# ALL THREE PLANTS ARE LIVE AGAIN (2026-09-05). Every defect this file catches is one factor of THE
# FRAME'S OWN SHAPE standing outside `uFitA`/`uFitB`, the one channel the host's crop-cancellation
# reaches — and this file's own header states the condition under which such a factor is worth
# nothing: "a seating that is wrong by the frame's ratio on a portrait work is wrong by nothing at
# all on a square one at a square frame". Between 2026-09-03 and 2026-09-05 the frame was the work's
# own box (PLAN row S-91), which made the frame's ratio the work's ratio at every door and left
# `planet`'s plant computing the same picture as its repair — cover-fitting a work into a box of its
# own aspect is identity, so the instrument's own `uFlatPP` fit WAS the host's. That row was written
# to say so and to redden the day the frame stopped being the box.
#
# The frame is the window again, so the two seatings part again and `planet` is judged the way
# `droste` and `overlay` always were: driven on the mixed-orientation pair, its pre-repair line
# reads 25.39 of 255 where the shipped file reads 0.38, against the same 6.0-of-255 threshold.
