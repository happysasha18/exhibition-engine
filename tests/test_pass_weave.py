...
# 2026-08-17 — REPOINTED, not softened. This row read `function passScoreFor` out of the client and
# so asserted the existence of the very road stage 0 removed under his 19:21 word (no pair table).
# The seam lane retired it and recorded the retirement in
# docs/immersive/briefs/reports/lanes/S0-seam-report.md; the retirement never reached this base, so
# the row stood red here on `u27-base` head 793c65c before this lane touched anything. What the row
# was actually for still holds and is carried over: the walk's road into a score is client code the
# site feeds with DATA, so a new score for a pair is a content change and never an engine rebuild.
# That road is now the composer's — the walk builds a request out of two work records and asks the
# one entry — and the row greps for it by name.
check("PASS-WEAVE the score's road into the walk needs no engine rebuild",
      all(n in (ROOT / "engine" / "client" / "01a-pass.js").read_text(encoding="utf-8")
          for n in ("function passComposeFor", "function passRequestFor")),
      "the site's pass record carries the per-work records; the bake passes the block through as "
      "data and the walk composes a passage out of two of them")
