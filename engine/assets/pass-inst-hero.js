...
(function () {
  ...
  function heroInstrument() {
    ...
    /* HOW LEGIBLY A WORK'S OWN TURN HAS TO READ before the window opens to the work's own count
       rather than to the module's own four folds. It is the composer's own DEVICE_LEGIBLE, the
       plainest meaning of a confidence — likelier right than wrong — and it is used here for the same
       reason it is used there: a reading under it is not a reading. */
    var FOLD_FLOOR = 0.5;
    ...
    function foldsOf(st) {
      ...
      if (score >= FOLD_FLOOR) {
        var lvl = Math.round(clamp(asked, 1, 4));
        return { level: lvl, wedges: Math.pow(2, lvl), score: score, measured: true,
                 from: "the pair's own measured order of turn" };
      }
      return { level: 4, wedges: 16, score: score, measured: false,
               from: "the module's own four folds — the pair's turn reads under the floor" };
    }
    ...
    var manifest = {
      ...
      handles: {
        ...
        folds: { min: 1, max: 4, def: 4, kind: "enum", step: 1,
                 ...
                 applied: { belowTheFloor: "the module's own four folds" } },
        foldsScore: { min: 0, max: 1, def: 0,
                      unit: "how confidently that order of turn reads",
                      reads: "structure.rotational.score, which is the gate: a pair whose turn reads "
                           + "under the floor has no order for a window to be cut at, and the "
                           + "module's own four folds stand with the reading saying so",
                      applied: { floor: FOLD_FLOOR } },
