...
(function () {
  ...
  function heroInstrument() {
    ...
    /* HOW THE KNEE IS READ BETWEEN TWO OF ITS OWN POINTS (2026-09-06). The two pieces above meet at
       ...
       is moving at: the knee is hinged at 0.61 rather than at a half, so a mirror about the middle
       does not put the two slopes at the join equal. Measured through the story `feelOf` rides, the
       speed stepped by 3.508 of the arc a unit of the hand at 0.74, where the fold carries the
       join's own mismatch with it. It is the same corner S-20 carried out of `matter`, `beat`,
       `gears`, `gates`, `adrift` and `waterline`, and Phase 7 out of `tilt` — whose knee is this
       knee in another key — and the jolt his word of 2026-08-28 named.
       ...
       tree already carries — and read back through the Fritsch-Carlson spline
       ...
       points exactly — the median itself among them, `feelKnee(0.5) = FEEL_C`, so the measured
    ...
    function feel(u) {
      return FEEL_D0 + (1 - FEEL_D0) * table(FEEL_Q, 0, u);
    }
    ...
    var manifest = {
      ...
      resources: { lean: { textures: 0, textureSlots: 2, framebuffers: 0, pingPong: 0,
                   programs: 1, passes: 1, bytesEstimate: 2666791, variant: "lean" },
                   standard: { textures: 0, textureSlots: 2, framebuffers: 0, pingPong: 0,
                               programs: 1, passes: 1, bytesEstimate: 10666791,
                               variant: "standard" },
                   rich: { textures: 0, textureSlots: 2, framebuffers: 0, pingPong: 0, programs: 1,
                           passes: 1, bytesEstimate: 42666791, variant: "rich" } },
      capabilities: ["webgl2"],
