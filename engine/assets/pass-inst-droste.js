...
(function () {
  ...
  function drosteInstrument() {
    ...
    /* HOW THE HAND'S OWN CURVE IS READ BETWEEN TWO OF ITS OWN POINTS (2026-09-06). Read as a bare
       ...
       own edge: the picture is held perfectly still under WIND_HOLD and past 1 - WIND_HOLD, and
       then leaves at the ramp's whole speed at once — measured, 3.333 of the dial a unit of the hand arriving
       ...
       `table`'s own division carries no `Math.max` guard where the ramp above carried one, and needs
       none: WIND_HOLD is a pinned 0.35 declared above, so `1 - 2 * WIND_HOLD` is 0.3 and can be
       neither nothing nor negative. */
    var FEEL_TANGENTS = [];
    ...
    function feelOf(u) {
      return table(FEEL_Q, WIND_HOLD, u);
    }
    ...
    return {
      ...
      // claim, not yet the fact. `feelOf` holds flat under WIND_HOLD and past 1 - WIND_HOLD and
      // leaves each dead band at the ramp's own nonzero speed at once, the same edge S-20 already
      // closed for `matter`/`beat`/`gears`/`gates`/`adrift`/`waterline`/`tilt`. Declared here so the
      // roll call reaches it and reports what it finds; repairing the edge is core logic and
