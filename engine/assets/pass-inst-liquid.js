...
(function () {
  ...
  function liquidInstrument() {
    ...
    // the water is flat and the line stands past the ceiling: the hand is spent there and the
    // standing work is the picture its source carries, to the point. That is what makes a door a
    // door and not a checkpoint.
    //
    ...
    /* HOW THE HAND'S OWN CURVE IS READ BETWEEN TWO OF ITS OWN POINTS (2026-09-06). Read as a bare
       ...
       leaves at the ramp's whole speed at once — measured, 1.111 of the dial a unit of the hand
       arriving out of nothing. That is the same corner S-20 carried out of `matter`, `beat`,
       `gears`, `gates`, `adrift` and `waterline`, and Phase 7 out of `tilt`: a corner no measurement
       ever had, and the jolt his word of 2026-08-28 named.
    ...
    // This hand's travel between its two dead bands is EVEN — equal movement, equal felt change,
    // which is the whole of what this curve claims — so its twenty-one shares are the hand's own
    // twenty-one equal marks. `table` re-applies the dead band the way `adrift`'s and `tilt`'s do.
