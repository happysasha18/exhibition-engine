...
(function () {
  ...
  //   · THE MIP CHAIN. The module builds one and asks for anisotropy, and its whole gradient estimate
  //     exists to pick a mip level that does not jump at a fold line. This host binds LINEAR with no
  //     chain at all, so `textureGrad` would have nothing to choose between and the estimate is inert
  //     arithmetic. It is therefore not carried, and the fact is written down rather than lost: what
  //     the missing chain costs is aliasing in the outer rings at a deep sample width, and it is a
  //     request to the host and not something an instrument can close from inside §1.2's fence.
  //
  // ------------------------------------------------------------------------------------------------
  ...
  function kaleidoscopeInstrument() {
    ...
    var manifest = {
      ...
      asks: {
        measure: "radial",
        both: { floor: "radial", says: "both works read radial over the collection's own cut-line "
                                     + "floor, so the fold opens a structure the pair carries" },
        arriving: { floor: "radial_tight", subType: "ring",
                    says: "the arriving work reads radial over the tight floor and its own subtype "
                        + "is rings, since rings are what open into a rosette and spokes turn" },
        reads: "structure.radial.score on both works, structure.radial.subType on the arriving one",
      },
